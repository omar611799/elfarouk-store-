    ...
    if (isSearching) {
      return (
        <div className="min-h-screen bg-obsidian-950 flex flex-col items-center justify-center">
          ...
          <p className="text-electric-400 mt-6 font-black animate-pulse text-[10px] uppercase tracking-[0.2em]">جاري البحث عن الفاتورة الرقمية...</p>
        </div>
      )
    }
    return <Navigate to="/" />
  ...
  return (
    <div className="min-h-screen bg-obsidian-950 flex justify-center py-10 px-4 sm:py-20">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-electric-500/5 blur-[120px] rounded-full animate-pulse-glow" />
      </div>
      ...
      <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full max-w-sm bg-white rounded-[2.5rem] overflow-hidden shadow-2xl relative z-10 flex flex-col border border-white/10">
        ...
        <div className="bg-obsidian-900 px-6 py-10 text-center relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-b from-electric-500/10 to-transparent" />
          <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="w-20 h-20 bg-electric-500/10 rounded-3xl flex items-center justify-center mx-auto mb-4 border border-electric-500/20 shadow-neon">
            <Store size={32} className="text-electric-400" />
          </motion.div>
          <h1 className="text-3xl font-black text-white tracking-tight font-display">الفاروق ستور</h1>
          <p className="text-electric-400 text-[10px] font-black tracking-[0.3em] uppercase mt-2 opacity-60">Auto Spare Parts</p>
        ...
        <div className="bg-white px-2 py-2">
            ...
            <div className="mx-2 mt-2 bg-slate-50/50 rounded-3xl p-6 border border-slate-100">
                ...
                <div>
                    ...
                    {inv.customerData?.carModel && (
                        <p className="text-electric-600 text-[10px] font-black uppercase tracking-widest mt-1">المعدات: {inv.customerData.carModel}</p>
                    )}
                </div>
            </div>
            ...
            <div className="px-6 py-8">
                <div className="flex items-center gap-2 mb-6">
                    <div className="w-1.5 h-4 bg-electric-500 rounded-full" />
                ...
                <div className="space-y-5">
                    ...
                    <div key={i} className="flex justify-between items-center group">
                        <div className="flex-1">
                            <p className="font-black text-slate-800 text-sm font-display leading-tight group-hover:text-electric-600 transition-colors">{item.name}</p>
            ...
            <div className="mx-2 mb-2 bg-obsidian-900 rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-electric-500/10 blur-[50px] -mr-16 -mt-16 group-hover:bg-electric-500/20 transition-all" />
                ...
                <div className="space-y-4 relative z-10">
                    ...
                    <div className="pt-6 space-y-3">
                        <button onClick={() => window.print()} className="w-full bg-electric-600 hover:bg-electric-500 text-white font-black py-4 rounded-2xl flex items-center justify-center gap-3 transition-all active:scale-95 text-[10px] uppercase tracking-widest shadow-lg shadow-electric-500/20">
                        ...
                        {inv.customerData?.phone && (
                            <Link to={`/portal/${inv.customerData.phone}`} className="w-full bg-white/5 hover:bg-white/10 text-slate-300 py-3.5 rounded-2xl flex items-center justify-center gap-2 transition-all border border-white/5 text-[9px] font-black uppercase tracking-widest">
                                <CheckCircle2 size={14} className="text-electric-400" /> سجـل الصيـانة الخـاص بك
                            </Link>
                        )}
                    </div>
                </div>
